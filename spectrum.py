...
async def async_whole_process(folder_name, model="gpt-4o-mini"):
    """
    ...
    async with AsyncOpenAI(api_key=hardcoded_api_key, base_url=api_base_url) as client:
        ...
        Q1_INPUT_CSV="result\spectrum\dataset_google_full_200.csv" 
        q2_input_file = None
...
def whole_process(folder_name, model):
    # This is the only place you need asyncio.run()
    asyncio.run(async_whole_process(folder_name, model=model))

# --- exmple usage ---
whole_process("result/spectrum/", TEST_MODEL)
