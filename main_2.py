...
from openai import AuthenticationError, OpenAIError, AsyncOpenAI, OpenAI # NEW
    ...
    async with semaphore:
        ...
        for role in roles_list_local:
            ...
            try:
                ...
                print(f"⚠️ API Error for role {role} in row (original data: {processed_row_data.get('id', 'N/A')}): {error_msg}")
...
async def main_only_recommend_batch(input_file,model="gpt-4o-mini"): # Changed to async def
    """
...
if __name__ == "__main__":
    ...
    input_csv_for_q2_batch = "r_recommendations_main5_20250525_120043_google.csv" # Replace with your actual input file
    target_model = "deepseek-chat" # Or "gpt-4o-mini" or any other model string
    # target_model = "gpt-4o-mini" 
    
    # Check if input file exists before running
    if os.path.exists(input_csv_for_q2_batch):
        asyncio.run(main_only_recommend_batch(input_csv_for_q2_batch, model=target_model))
    else:
        print(f"❌ Input file for Q2 batch processing not found: {input_csv_for_q2_batch}")
